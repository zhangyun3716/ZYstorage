//  PutinstorageViewController.m  5.0w-1.0w-1.5w=2.0w
//  flexiumstorage    5.0W + 0.05w - 1.0w - 1.5w = 2.5w
//  Created by flexium on 2017/4/28  5.0w - 1.0w - 2.0w = 2.0w
//  Copyright © 2017年 FLEXium. All rights reserved.
//@"http://ksrv-sap-qas.flexium.local:8000/sap/bc/srt/rfc/sap/zsrpp005/888/service/binding"
//  
...
@interface PutinstorageViewController ()<NSXMLParserDelegate,UITableViewDataSource,UITableViewDelegate>

#define CEll1 @"CELL1"

@property (strong, nonatomic) IBOutlet UIButton *beginscan;/**< 开始扫描按钮 */

- (IBAction)beginscan:(id)sender;/**< 开始扫描开关 */
...
@property (nonatomic, strong) NSMutableArray *inboxmessage;/**< 內箱信息array */
...
@property (strong, nonatomic) NSString *sapgoodfanhui;/**< sap成功返回 */
...
@implementation PutinstorageViewController
...
- (void)viewDidLoad {
    ...
    //開始掃描
    self.beginscan.layer.borderWidth = 1.0;
    self.beginscan.layer.borderColor = [UIColor grayColor].CGColor;
    self.beginscan.layer.cornerRadius=10;
    self.beginscan.layer.masksToBounds=YES;
...
- (IBAction)beginscan:(id)sender {
    ...
    //扫描料号
        ...
        [[NSUserDefaults standardUserDefaults] setValue:@"掃描料號" forKey: @"textlable"];
        
        [[ZYScannerView sharedScannerView] showOnView:self.view block:^(NSString *str) {
            
           
            self.ExZivnos=nil;
//            [self checkworkno:str];
             [self checkinboxworksno:str];
             self.navigationController.navigationBarHidden=NO;//上方标题栏设置为不隐藏
     ...
     //扫描彩盒
        ...
        [[NSUserDefaults standardUserDefaults] setValue:@"掃描彩盒号" forKey: @"textlable"];
        
        [[ZYScannerView sharedScannerView] showOnView:self.view block:^(NSString *str) {
            
            self.inboxnolable.text=str;
            self.teshubiaoji=0;
            self.teshubiaoji2=0;
            self.fanhuibiaoji=0;
             self.teshubiaoji=0;
            _charusuoyoubiaoji=0;
...
//        [self.worknomesage addObject:_returnresult] ;
    ...
    if ([_currentElementName isEqualToString:@"checkinboxnoResult"]) {
        ...
        _returnresult =[_returnresult stringByAppendingString:string] ;
        self .inboxmessage =[[NSMutableArray alloc]init];

        [self.inboxmessage addObjectsFromArray:[self.returnresult componentsSeparatedByString:@";"]];
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (_qingqiutype==0) {
            ...
            if ( [self.peoplemessage[0] isEqualToString:@"OK"]) {
                ...
                [self.beginscan setTitle:@"掃描料號" forState:UIControlStateNormal];
        ...
        //2.工单編號
            ...
            if (_worknomesage.count>2) {
                ...
                [self.beginscan setTitle:@"扫描彩盒" forState:UIControlStateNormal];
...
//                self.shulianglable.text=[NSString stringWithFormat:@"0/%d",self.zhongshu];
                self.shulianglable.text=[NSString stringWithFormat:@"總:0/%d 良:0/%d \n不良:0/%d",self.zhongshu,self.goodzhongshu,_badzhongshu];
                ...
                [self.beginscan setTitle:@"扫描料號" forState:UIControlStateNormal];
...
//                    [self tixing:@"工單良品數量預加已滿無法掃描加入請確認彩盒信息或工單信息" type:@"NG"];
                ...
                [self.beginscan setTitle:@"掃描彩盒號" forState:UIControlStateNormal];
    ...
    //插入mes数据库
        ...
        if([self.insertmessage[0]isEqualToString:@"OK"]){
            ...
            [self.beginscan setTitle:@"扫描工卡" forState:UIControlStateNormal];
...
-(void)beginjuhua{
    ...
    [testActivityIndicator setFrame :CGRectMake(100, 200, 100, 100)];//不建议这样设置
...
//    cell1.caihehao.text=self.allcaihemessagearray[indexPath.row][0];
//    [self.allcaihemessagearray.count-indexPath.row-1]
...
-(void)qingkongallshuju{
     ...
     self.sapreturnEXMSG=nil;/**<  sap返回错误信息 */
    
     self.arraylist=nil;/**<  暂时存储的地址 */
   
     self.empno=nil;/**<  工卡号 */
    
     self.empid=nil;/**< 工卡id */

     self.empname=nil;/**< 人员姓名 */
    
     self.boxno=nil;/**<  箱号 */
    
     self.storageliaohao=nil;/**<  储位料号标记 */
    
     self.storagechuliang=nil;/**<  储位储量标记 */

     self.storagetype=nil;/**<  储位类型标记 */
    
     self.empnolable.text=nil;/**<  工号lable标记 */
    
     self.intostorageordernolable.text=nil;/**<  工单lable */
    
     self.inboxnolable.text=nil;/**<  内箱lable */
    
     self.shulianglable.text=nil;/**<  数量lable */
    
     self.fanhuimessage.text=nil;/**<  返回信息lable */
    
     self.tiaochu=0;/**<  循环跳出标记 */
    
     self.sapgoodfanhui=nil;/**<  sap成功返回数据 */
